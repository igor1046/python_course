...
class ResourcesMenu:
    ...
    def get_topics(self) -> list[str]:
        ...
        resources_button = self.page.get_by_role("button", name=self.RESOURCES_BUTTON_NAME)
        ...
        topics_section = self.page.get_by_text(self.TOPICS_SECTION_TEXT, exact=False).first
        expect(topics_section).to_be_visible()

        # Берем ближайший список после заголовка секции
        topic_links = topics_section.locator("xpath=following::ul[1]").locator(self.TOPIC_LINKS)

        return [text.strip() for text in topic_links.all_text_contents() if text.strip()]
